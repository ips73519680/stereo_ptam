...
class KittiEvalOdom():
    ...
    def compute_overall_err(self, seq_err):
        ...
        if seq_len > 0:
            ...
            return 0, 0
# ---------------------------------------------------------------------------
    def plot_trajectory(self, poses_gt, poses_result,c_poses_result, seq):
...
# ---------------------------------------------------------------------------
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        self.plot_path_dir = result_dir + "/compare_plot_path"
        ...
        if not os.path.exists(self.plot_path_dir):
            ...
            print('os.makedirs(self.plot_path_dir)')
        ...
        for i in self.eval_seqs:
            ...
            c_file_name = '{:02}_c.txt'.format(i)

            poses_result = self.load_poses_from_txt(result_dir+"/"+file_name)
            c_poses_result = self.load_poses_from_txt(result_dir+"/"+c_file_name)
            ...
            # Plotting
            self.plot_trajectory(poses_gt, poses_result,c_poses_result, i)            

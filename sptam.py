...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, help='dataset path', 
        default='/home/b08505024/stereo_ptam-master/dataset/KITTI/dataset/sequences/02')
    ...
    if args.dataset.lower() == 'kitti':
        params = ParamsKITTI(config)
        # params = ParamsKITTI()
        dataset = KITTIOdometry(args.path,config)
    ...
    if not os.path.isdir(result_dir):
        os.mkdir(result_dir)
    result_path = os.path.join(result_dir,args.path[-2:])
    f = open(result_path+'.txt', 'w')
    ...
    # for i in tqdm(range(len(dataset))[:150]):
        ...
        flatten_str = " ".join(('%s' %id for id in flatten_list))+'\n'
        f.write(flatten_str)
    ...
    print('sequence{}'.format(args.path[-2:]))
    print('loopclose removed: ',(sptam.loop_closing==None))
    print('config : ',config)
    f.close()
